...
const PheDuyet = () => {
  ...
  // 2. Thêm hàm format hiển thị hồ sơ
  const formatDisplayHoSo = (listHosoId) => {
    if (!listHosoId || listHosoId.length === 0) return "N/A";

    return listHosoId
      .map((id) => {
        const hoso = hoSoList.find((hs) => hs.id === id);
        if (!hoso) return "";
        return `Tập ${hoso.taphoso.replace("tap", "")} - ${
          hoso.loaihoso || ""
        }`;
      })
  ...
  // const formatTapHoSo = (taphoso) => {
  //   if (!taphoso) return '';
  //   if (taphoso.startsWith('tap')) {
  //     return `Tập ${taphoso.substring(3)}`;
  //   }
  //   return taphoso;
  // };
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>
                          ...
                          {item.loaipheduyet === "hoso" &&
                            (item.listHosoId.length > 0
                              ? formatDisplayHoSo(item.listHosoId)
                              : "N/A")}
                          {item.loaipheduyet === "tintuc" &&
                            (tinTucList.find((tt) => tt.id === item.tintucId)
                              ?.tieude ||
